%%% TRAJECTORY_F_PM_THM1_S5_0.05_0.05.M - Trajectory Analysis for Stable Point S5 vs. F_PM %%%
clear; clc; close all;
...
for i = 1:length(f_PM_values)
    ...
    % Create the legend label for f_PM
...
%%% === 4. PLOT ANNOTATIONS
%%% ===================================================================
% Plot the initial starting point 'S5'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 12, 'MarkerFaceColor', 'r');
text(initial_condition(1), initial_condition(2) + 0.04, '$S_5$', 'FontSize', 12, 'HorizontalAlignment', 'center', 'Interpreter', 'latex');
...
title({'Trajectory Sensitivity to $f_{PM}$ from Stable Initial Point $S_5$ (0.05, 0.05)', '--- Theorem 1'},'Interpreter','latex', 'FontSize', 16);
grid on; box on;
...
outputFileName = 'Trajectory_f_PM_thm1_S5_0.05_0.05.pdf';
