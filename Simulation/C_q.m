...
% and plots the time evolution of the second state variable, q.
...
for k = 1:length(n_values)
    ...
    % The initial condition is [p(0), q(0)] = [0.8, 0.85]
    [T, Y] = ode45(@(t, y) dynamic_system(t, y, n), [0 1000], [0.8, 0.85]);
    
    % Generate a legend label for the current n value, using TeX for subscript
    legend_label = sprintf('n_{%d} = %.3f', k, n);
    
    % Plot the time evolution of q (the second state variable, Y(:,2))
    plot(T, Y(:,2), '-',...
        'Color', colors(k,:),...
        'LineWidth', 2,...
...
ylim([0 1]);         % Set the y-axis (probability q) limits
xlabel('Time (t)');
ylabel('q');
grid on;
...
outputFileName = fullfile(outputPath, 'F7_2.eps');
